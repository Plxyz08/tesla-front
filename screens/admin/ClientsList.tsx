...
import { Searchbar, Card, Button, Chip, FAB, Dialog, Portal, Menu } from "react-native-paper"
...
export default function ClientsList() {
  ...
  // Nueva función para exportar la lista de clientes como PDF
  const exportClientsToPDF = async () => {
    try {
      ...
      const fileName = `Lista_clientes_${formattedDate}`

      // Generar el HTML para el PDF
      const html = generateClientsListHTML(filteredClients)

      // Usar el servicio de PDF existente para generar el PDF
      const { filePath, success } = await generatePDF({
        html,
        fileName,
      })
      ...
      if (success && filePath) {
        ...
        Alert.alert("Éxito", "Lista de clientes exportada correctamente")
      } else {
        throw new Error("No se pudo generar el PDF")
      }
    } catch (error) {
      console.error("Error al exportar clientes:", error)
      Alert.alert("Error", "No se pudo exportar la lista de clientes. Por favor, intente nuevamente.")
    } finally {
  ...
  const generateClientsListHTML = (clientsList: Client[]) => {
    ...
    const clientRows = clientsList
      ...
      <tr>
        ...
        <td>${client.totalCuentaCliente || "N/A"}</td>
      </tr>
    `,
      )
      .join("")
    ...
    return `
      ...
      <html>
        ...
        <body>
          ...
          <table>
            ...
            <tbody>
              ${
                ...
                `
                <tr>
                  <td colspan="8" style="text-align: center; color: #6b7280;">No hay clientes disponibles</td>
                </tr>
              `
              }
            </tbody>
          ...
          <div class="footer">
            <p>Este documento es confidencial y para uso interno de Tesla Lift.</p>
  ...
  // Función para generar PDF (reutilizando la lógica existente)
  const generatePDF = async (config: { html: string; fileName: string }) => {
    try {
      // Usar la función existente de PDFGenerator
      return await PDFGenerator.generateAccountStatementPDF([], [], {
        customerInfo: {
